#!/usr/bin/env bash
# Run with: pixi run bash diag/test_control_stop.sh (Linux, no hardware).
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
: "${CONDA_PREFIX:?Run this offline test under pixi run}"
TEST_DIR="$(mktemp -d /tmp/pnp7-stop-test-XXXXXX)"
...
"${CXX:-$CONDA_PREFIX/bin/g++}" -O2 -std=c++17 -Wall -Wextra -pthread \
  ...
  "$HERE/diag/test_control_stop.cpp" -o "$TEST_DIR/test" \
